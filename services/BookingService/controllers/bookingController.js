// Controller factory for BookingService route handlers
...
module.exports = ({ pool, redisClient, getIO }) => {
  return {
    ...
    getRefundBookings: async (req, res) => {
      try {
    ...
    getRefundBookingById: async (req, res) => {
      try {
    ...
    getRefundBookingsByBookingId: async (req, res) => {
      try {
    ...
    // Get booking by ID (with seat_ids)
    ...
    updateTotalPrice: async (req, res) => {
      ...
      if (total_price === undefined || isNaN(total_price)) {
        return res
          .status(400)
          .json({ error: "Invalid or missing total_price" });
      }
    ...
    // Get locked seats for a showtime (from cache or DB)
